...
If we denote set 1 as $S1$ and set 2 as $S2$, the Jaccard coefficient is defined as:

$$ Jaccard = \frac {\bigcap(S1, S2)}{\bigcup(S1, S2)} $$

Please implement code to calculate the Jaccard coefficient between CpG island and promoters and the Jaccard
coefficient between CpG island and exons.

Hint: to calculate the intersection of S1 and S2, use `intersect()` function. To calculate the union of S1 and S2,
